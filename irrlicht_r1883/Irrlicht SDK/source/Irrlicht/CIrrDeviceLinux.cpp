...
// >> add by uirou for IME Window start
void CIrrDeviceLinux::focusIn(void)
{
	if(im != NULL)
		XSetICFocus(ic);
}
void CIrrDeviceLinux::focusOut(void)
{
	if(im != NULL)
		XUnsetICFocus(ic);
}
...
bool CIrrDeviceLinux::setupXIM(void)
{
    // Use default locale
    ...
    fs = XCreateFontSet(display, "-*-*-*-R-Normal--14-130-75-75-*-*", // XXXX magic word...
...
// << add by uirou for IME Window end
			...
			case KeyPress:
				{
					SKeyMap mp;
// >> modified by uirou for IME Window start
//					if(im == NULL){
					char buf[8]={0};
...
/*
						...
						Status status;
						int i;
						int len = XwcLookupString(ic, &event.xkey, buf, sizeof(buf),
							&k, &status);
						for(i = 0; i < len; i++){
							irrevent.EventType = irr::EET_KEY_INPUT_EVENT;
							mp.X11Key = k;
							...
							if(idx != -1){
								...
								irrevent.KeyInput.Key = irr::KEY_ACCEPT;
							}
							irrevent.KeyInput.PressedDown = (event.type == KeyPress);
...
//					irrevent.KeyInput.Shift = (event.xkey.state & ShiftMask) != 0;
//					postEventFromUser(irrevent);
// << modified by uirou for IME Window end
				}
				break;
